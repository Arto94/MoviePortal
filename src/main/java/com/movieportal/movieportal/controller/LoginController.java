...
public class LoginController {
    ...
    public String saveUser(@Valid @ModelAttribute("user") User user, BindingResult result, @RequestParam("picture") MultipartFile multipartFile) throws IOException {
        ...
        if (userRepository.findOneByEmail(user.getEmail()) == null) {
            ...
            String message = String.format("Hi %s, You are successfully registered to our cool portal. Please visit by  <a href='http://localhost:8088/verify?token=%s' this link to verify your account", user.getName(), token);
